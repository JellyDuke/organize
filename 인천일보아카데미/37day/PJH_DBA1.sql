...
--[종류별상품목록] 선택한 종류 "식품"기능
SELECT *
FROM GOODS
WHERE GTYPE = '식품' AND GSTOCK > 0;

--[상품주문]기능 - 바나나우유 상품 구매, 5개 구매
--1.ORDERS 테이블에 INSERT
INSERT INTO ORDERS(ODCODE, ODMID, ODGCODE, ODQTY, ODDATE)
VALUES('G0001','PJH','G0115', 5, SYSDATE);

INSERT INTO ORDERS(ODCODE, ODMID, ODGCODE, ODQTY, ODDATE)
VALUES('G0002','PJH','G2322', 1, SYSDATE);

INSERT INTO ORDERS(ODCODE, ODMID, ODGCODE, ODQTY, ODDATE)
VALUES('G0003','PJH','G1357', 5, SYSDATE);

INSERT INTO ORDERS(ODCODE, ODMID, ODGCODE, ODQTY, ODDATE)
VALUES('G0004','PJH','G1235', 1, SYSDATE);

INSERT INTO ORDERS(ODCODE, ODMID, ODGCODE, ODQTY, ODDATE)
VALUES('G0005','PJH','G2223', 5, SYSDATE);

INSERT INTO ORDERS(ODCODE, ODMID, ODGCODE, ODQTY, ODDATE)
VALUES('G0006','PJH','G2211', 10, SYSDATE);
...
SELECT MEMBERS.MNAME, GOODS.GNAME, GOODS.GPRICE, ORDERS.ODQTY,(ODQTY * GPRICE), ORDERS.ODDATE
FROM MEMBERS INNER JOIN ORDERS ON MEMBERS.MID = ORDERS.ODMID
             INNER JOIN GOODS ON ORDERS.ODGCODE = GOODS.GCODE;
...
--주문 금액이 가장 많은 회원 정렬 - 회원아이디, 총구매액수
SELECT OD.ODMID, SUM(OD.ODQTY * G.GPRICE) AS A
...
FROM MEMBERS M INNER JOIN (SELECT ODMID, COUNT(*) AS TOTALORDERS
                FROM ORDERS
                GROUP BY ODMID
                ORDER BY TOTALORDERS DESC) OD ON M.MID = OD.ODMID;
                
SELECT *
FROM MEMBERS M LEFT JOIN (SELECT ODMID, COUNT(*) AS TOTALORDERS
                FROM ORDERS
...
--판매량이 많은 순(인기제품)
--1. ORDERS
SELECT ODGCODE , SUM(ODQTY) AS A
FROM ORDERS
GROUP BY ODGCODE
ORDER BY A DESC;
--2.
SELECT *
FROM GOODS G INNER JOIN(SELECT ODGCODE , SUM(ODQTY) AS A
                        FROM ORDERS
                        GROUP BY ODGCODE) OD
...
SELECT G.*, NVL(OD.A,0)
FROM GOODS G LEFT OUTER JOIN(SELECT ODGCODE , SUM(ODQTY) AS A
                        FROM ORDERS
                        GROUP BY ODGCODE) OD
                        ON G.GCODE = OD.ODGCODE
ORDER BY NVL(OD.A,0) DESC;

--======================================06.02================================================

--OUTER JOIN
--INNER JOIN - MEMBERS, ORDERS
SELECT *
FROM MEMBERS M INNER JOIN ORDERS OD ON M.MID = OD.ODMID;

--OUTER JOIN - MEMBERS, ORDERS
...
-- 판매량이 많은 순으로 정렬해서 상품 목록을 보여주세요
--1. 어떤제품이 얼만큼 판매가 되었는지 조회 - ORDERS
SELECT ODGCODE , SUM(ODQTY) AS A
FROM ORDERS
GROUP BY ODGCODE
ORDER BY A DESC;
...
FROM GOODS G LEFT OUTER JOIN(SELECT ODGCODE , SUM(ODQTY) AS A
                        ...
                        GROUP BY ODGCODE) OD
                        ON G.GCODE = OD.ODGCODE
ORDER BY NVL(OD.A,0) DESC, G.GPRICE;

--매출액이 가장 높은 상품 순으로 정렬
--1. 상품별 매출액 조회 - ORDERS.ODQTY * GOODS.GPRICE
SELECT OD.ODGCODE, SUM(OD.ODQTY * G.GPRICE) AS A
FROM ORDERS OD, GOODS G
WHERE OD. ODGCODE = G.GCODE
GROUP BY OD.ODGCODE;

--2.상품을 매출액 순으로 정렬
SELECT G.*, NVL(OD.A,0) AS B
FROM GOODS G LEFT OUTER JOIN(SELECT OD.ODGCODE, SUM(OD.ODQTY * G.GPRICE) AS A
                             FROM ORDERS OD, GOODS G
                             WHERE OD. ODGCODE = G.GCODE
...
--가상테이블(VIEW)
CREATE OR REPLACE VIEW TESTBIEW1
...
--ORDERS, GOODS, MEMBERS 테이블을 INNER JOIN한 VIEW 생성
CREATE OR REPLACE VIEW TESTVIEW1
AS (SELECT * FROM GOODS G INNER JOIN ORDERS OD ON G.GCODE = OD.ODGCODE 
                          INNER JOIN MEMBERS M ON M.MID = OD.ODMID) ;
SELECT GNAME, MNAME
FROM TESTVIEW1
...
CREATE TABLE CRUDTEST(
    NUMTEST NUMBER,
    CHARTEST NVARCHAR2(5),
    DATETEST DATE
);
ALTER TABLE CRUDTEST
ADD CONSTRAINT PK_CRUD PRIMARY KEY(NUMTEST);

INSERT INTO CRUDTEST(NUMTEST,CHARTEST,DATETEST)
VALUES( 1, '테스트1', SYSDATE);

select * from CRUDTEST;
DELETE FROM CRUDTEST;
COMMIT;
