...
//        System.out.println(sb.toString());
	...
	public String getBusnodeList(String routeid) throws IOException{
        ...
        rd.close();
        conn.disconnect();
        JsonObject routeidJson = JsonParser.parseString(sb.toString()).getAsJsonObject();
        JsonArray routeidarr = routeidJson.get("response").getAsJsonObject().get("body").getAsJsonObject().get("items").getAsJsonObject().get("item").getAsJsonArray();	
        System.out.println(routeidJson);

        return new Gson().toJson(routeidarr);  
	}
