...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		case "/memLogin":
			...
			System.out.println("로그인 한 아이디" + loginId +"비밀번호"+loginPw);
			...
			if(memInfo == null) {
				...
				response.sendRedirect(path +"/Fail.jsp" +"?msg=" + URLEncoder.encode("아이디 또는 비밀번호가 일치하지 않습니다.","UTF-8"));
			}else {
		...
		case "/myInfo" :
			...
			//myInfo.jsp
			if(infoId == null) {
				response.sendRedirect(path +"/Fail.jsp" +"?msg=" + URLEncoder.encode("잘못된 요청입니다.","UTF-8"));
