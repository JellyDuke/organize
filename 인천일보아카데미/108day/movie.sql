...
--2. 영화 + 예매 수 조회
SELECT MV.*, NVL(RE.RECOUNT,0) AS RECOUNT
FROM MOVIES MV
     LEFT JOIN (SELECT MVCODE, COUNT(MVCODE) AS RECOUNT
                FROM RESERVES
                GROUP BY MVCODE
                ) RE
...
WHERE RECODE IN(SELECT RECODE
                FROM RESERVES
                WHERE MVCODE = 'MV00025');

SELECT *
FROM REVIEWS;
INSERT INTO REVIEWS(RVCODE, RECODE, MID, RVCOMMENT, RVDATE)
		VALUES('RV00001', 'RE00021', 'LCY','ASD', SYSDATE);
DELETE FROM REVIEWS;
commit;
