        	...
        	<div class="row" style="height: 200px; border: 1px solid black;">
        		...
        		<div class="col-lg-6 col-md-6">
        			<div class="d-flex m-2"> 
        				<div class="m-1"> 
        				<img class="rounded-circle" src="${review.MPROFILE }">
        				<c:choose>
                            <c:when test="${review.MPROFILE == 'YC' }">
								<c:choose>
						...
						<div class="fw-bold">${review.MID } <span class="text-sm">${review.REDATE }</span> </div>
        				<div>
        					<textarea>${review.RVCOMMENT }</textarea>
        				</div>
        			</div>
        		</div>

        		<c:if test="${sessionScope.loginId == review.MID }">
        		<a href="/deletReview?${review.RVCODE }&mvcode=${movie.mvcode}" class="btn btn-danger w-50">관람평 삭제</a>
        		</c:if>
				<%--mav.setViewName("redirect:/detailMovie?mvcode=??????") --%>
        		</c:forEach>
        	</div>
