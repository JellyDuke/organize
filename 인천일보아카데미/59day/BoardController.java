...
//			SELECT * FROM BOARDS WHERE BSTATE = '1';
		...
		case "/boardDelete" :
			...
			int deleteRs = 0;
			int deleteBno = Integer.parseInt(request.getParameter("deleteBno"));
			//UPDATE BOARDS SET BSTATE = '0' WHERE BNO = ?
			deleteRs = bsvc.getUpdateView(deleteBno);
			if(deleteRs > 0) {
				response.sendRedirect(path + "/Success.jsp"
						+ "?msg=" + URLEncoder.encode(deleteBno+"번 게시글 삭제 성공", "UTF-8")
						+ "&url=" + URLEncoder.encode(path + "/boardList", "UTF-8"));
		...
		case "/boardSearch":
			...
			String searchTilte = request.getParameter("searchTilte");
			System.out.println("검색어 : " + searchTilte);
			
			//SELECT * FROM BOARD WHERE BSTATE = '1' AND BTITLE LIKE '%2023%'
			
			ArrayList<Board> searchBList = bsvc.getSearchBoard(searchTilte);
