...
//			}
			...
			String gtype = gtypeList.get(selectType).getGtype(); //scan로 숫자 선택을 gtype에 저장, 기타가 0번일때 0번을 입력시 0번을 gtype에 저장
...
//	상품주문
	public void goodsOrder() {
		...
		int numCode = Integer.parseInt(maxOdcode.substring(1)) + 1; //Interger.parseInt -> 문자열을 인트형 으로.. -> 0000 문자를 0000 숫자로 선언
...
//		od.setOdqty(odqty);
	...
	public void orderList() {
		...
		case 1:
			...
			if(orderList == null ) {
				...
				for(int i = 0; i < orderList.size(); i++) {
					System.out.print(orderList.get(i).get("ODCODE")+ " ");
		...
		case 2:
			orderList = shopdao.orderList2(loginId,select);
			if(orderList == null ) {
				System.out.println("다시 선택해주세요.");
				return;
			} else if(orderList.size() == 0){
