...
<html>
	...
	<script type="text/javascript">
		...
		function printReplyList(reList) {
			...
			reWriter.innerText = "작성자: "+reInfo.remid;
			replyDiv.appendChild(reWriter);
			
			let reDate = document.createElement('p');
			reDate.innerText = "작성일: "+reInfo.redate;
			replyDiv.appendChild(reDate);
				

			if(reInfo.remid == loginId){
			let reDelete = document.createElement('button');
			reDelete.innerText = "댓글 삭제";
			reDelete.setAttribute('onclick','delReply("'+reInfo.renum+'")');
			replyDiv.appendChild(reDelete);
			}
